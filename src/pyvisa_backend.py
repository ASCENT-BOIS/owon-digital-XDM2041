...
AGILENT_34401A_PROFILE = {
    ...
    "modes": {
        "VDC": {"conf": "CONF:VOLT:DC {range}", "measure": "MEAS:VOLT:DC?"},
        "VAC": {"conf": "CONF:VOLT:AC {range}", "measure": "MEAS:VOLT:AC?"},
        "A": {"conf": "CONF:CURR:DC {range}", "measure": "MEAS:CURR:DC?"},
        "OHM": {"conf": "CONF:RES {range}", "measure": "MEAS:RES?"},
        "HZ": {"measure": "MEAS:FREQuency?"},
    },
    "supports_capacitance": False,
    "units": {"VDC": "V", "VAC": "V", "A": "A", "OHM": "\u03a9", "HZ": "Hz"},
}
...
class PyVISAMultimeter(Multimeter):
    ...
    def set_mode(self, mode: str) -> None:
        # First, let the base class update the internal state
        super().set_mode(mode)

        # If we are connected to a real meter, send the SCPI configure command immediately
        if self.connected and self._inst:
            conf_map = {
                "VDC": "CONFigure:VOLTage:DC",
                "VAC": "CONFigure:VOLTage:AC",
                "A": "CONFigure:CURRent:DC",
                "OHM": "CONFigure:FRESistance",
                "HZ": "CONFigure:FREQuency",
                "CAP": "CONFigure:CAPacitance",
            }
            scpi_cmd = conf_map.get(mode)
            if scpi_cmd:
                try:
                    self.write(scpi_cmd)
                except Exception:
    ...
    def measure_raw(self) -> Tuple[float, str]:
        ...
        #             rng = (
        #                 "AUTO"
        #                 if (self.range is None or str(self.range).upper() == "AUTO")
        #                 else str(self.range)
        #             )
        ...
        # resp = self.query(q)
        val = float(self._inst.query("MEAS?"))
        unit_map = {
            "VDC": "V",
            "VAC": "V",
            "A": "A",
